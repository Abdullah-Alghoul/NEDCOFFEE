...
class account_move_reversal(orm.TransientModel):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        res = super(account_move_reversal, self).default_get(cr, uid, fields, context=context)
        this = self.pool.get('account.move').browse(cr,uid,context['active_id'])
        if this.reversal_id:
            error = '''reversal %s  is exist''' %(this.reversal_id.name)
            raise osv.except_osv(unicode('Error!', 'utf8'), unicode(error, 'utf8'))
        users = self.pool.get('res.users').browse(cr, uid, uid)
        reversal = users.company_id.reverse_journal_id
        res.update({'journal_id': reversal.id,'date': this.date_reversed})
        return res
