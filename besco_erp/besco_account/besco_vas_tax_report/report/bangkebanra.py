...
class Parser(report_sxw.rml_parse):
    ...
    def get_line(self, tax=None):
        ...
        sql_vat_invoice = ''
        if self.show_invoice in ['vat','all'] and tax != None:
            result = self.pool.get('account.tax').search(self.cr, self.uid, [('amount','=',tax),
                                                                             ('type_tax_use','=','sale'),
            ...
            from_invoice = ''
            from_voucher = ''
            if (len(self.tax_ids) and report_tax_id in self.tax_ids) or not len(self.tax_ids):
        ...
        if self.show_invoice == 'all':
            sql = len(sql_untax_invoice) and sql_untax_invoice or ''
            sql = (len(sql_untax_invoice) and len(sql_vat_invoice)) and sql + ' UNION ALL ' + sql_vat_invoice or sql + sql_vat_invoice
            if len(sql):
                sql = 'SELECT * from ( ' + sql + ')x ORDER BY date_invoice, reference, tax'
        else:
            if self.show_invoice == 'vat':
                sql = sql_vat_invoice
            if self.show_invoice == 'untaxed':
