...
class sql_tb_closing_fiscalyear(osv.osv):
    _name = "sql.tb.closing.fiscalyear"
    _auto = False
    
    def init(self, cr):
        self.fi_tb_closing_fiscalyear(cr)
        return True
    
    def fi_tb_closing_fiscalyear(self,cr):
        #THANH: Try to delete old function which has 3 parameters, newest has 4
        sql = '''
        delete from pg_proc where proname = 'fi_tb_closing_fiscalyear';
        commit;
        
        CREATE OR REPLACE FUNCTION fi_tb_closing_fiscalyear(IN date, IN date, IN integer, IN date, IN INT[], 
            OUT coa_id integer, OUT coa_code character varying, OUT coa_name character varying, OUT internal_type character varying, OUT acc_level integer, 
        ...
        BEGIN
            for rec_trial in 
                Select  distinct bal.acc_id, bal.code, bal.name, bal.internal_type, bal.acc_level,
                ...
                From (
                    select bal.acc_id, bal.code, bal.name, bal.internal_type, bal.acc_level, 
                    ...
                    from (
                        /*    Lay so du dau ky    */
                        select acc.id acc_id, acc.code, acc.name, ajn.internal_type, acc.level acc_level,
                            sum(aml.debit) begin_dr, sum(aml.credit) begin_cr,
                            0 period_dr, 0 period_cr,
                            0 ac_dr_val, 0 ac_cr_val,
                            
                            sum(case when aml.debit != 0 then abs(abs(aml.second_amount))::numeric 
                                else 0.0::numeric
                            end)  begin_dr_2rd,
                            sum(case when aml.credit != 0 then abs(abs(aml.second_amount))::numeric 
                                else 0.0::numeric
                            end) begin_cr_2rd,
                        ...
                        where amh.company_id = $3
                            and aml.date = coalesce((select am.date
                                                        ...
                                                        limit 1),'2000-01-01')
                        group by acc.id, acc.code, acc.name, ajn.internal_type, acc.level                                    
                    ...
                    group by bal.acc_id, bal.code, bal.name, bal.internal_type, bal.acc_level
                ) bal
            ...
            loop
                ...
                internal_type = rec_trial.internal_type;
