...
class Parser(report_sxw.rml_parse):
    ...
    def get_format_amount(self, obj):
        ...
        amount =0.0
        for o in obj:
            amount += round(o.amount, o.currency_id.round_decimal_places)
            for fee in o.payment_fee_lines:
                amount += round(fee.amount, o.currency_id.round_decimal_places)
        string = self.pool.get('res.lang').format(self.cr, self.uid, [lang_ids[0]], '%d', \
                                                amount, grouping=True, monetary=True)
        return string
    
    def get_string_amount(self, obj):
        amount = 0.0
        first_partner_id = False
        for o in obj:
            if not first_partner_id:
                first_partner_id = o.partner_id.id
            if first_partner_id and first_partner_id != o.partner_id.id:
                raise UserError(_("Multi payments must have the same Partner '%s'.")%(o.partner_id.name))
            amount += o.amount
            for fee in o.payment_fee_lines:
                amount += fee.amount
    ...
    def get_company_bank(self, o):
        if o.state != 'posted':
            raise UserError(_("Bank Payment '%s' must be posted before printing !")%(o.name))
        if not self.company_bank:
            ...
            self.company_bank.append(bank_journal.bank_id.bic or '')
        return self.company_bank
    
    def bank_number(self):
        bank_number = self.company_bank[1]
        number = ''
        for i in range(0, len(bank_number) - 0):
