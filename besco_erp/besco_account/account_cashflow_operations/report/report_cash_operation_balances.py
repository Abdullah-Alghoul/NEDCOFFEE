...
class Parser(rml_parse):
    ...
    def get_header(self):
        ...
        sql = _(u'''
            ...
            where cd.type = '%(type)s' and cd.state = 'confirm'
    ...
    def get_vay_rate(self, cash_id, remain_amount):
        cash = self.pool.get('account.cash.operation').browse(self.cr, self.uid, cash_id)
        vay_rate = 1
        self.currency_value = remain_amount
        self.second_currency_value = remain_amount
        self.second_rate = 1
        
        if cash.currency_id != cash.company_id.currency_id and remain_amount:
#             ex_rate, rate = self.pool.get('res.currency')._get_conversion_rate(self.cr, self.uid, cash.currency_id, cash.company_id.currency_id, context={'date':cash.date_start})
            convert = cash.currency_id.with_context({'date':cash.date_start}).compute(remain_amount, self.company.currency_id)
            vay_rate = self.company.currency_id.round(convert > remain_amount and convert / remain_amount or remain_amount / convert)
            self.currency_value = convert
            
        if cash.company_id.second_currency_id and cash.currency_id != cash.company_id.second_currency_id and remain_amount:
#             ex_rate, rate = self.pool.get('res.currency')._get_conversion_rate(self.cr, self.uid, cash.currency_id, cash.company_id.second_currency_id, context={'date':cash.date_start})
            convert = cash.currency_id.with_context({'date':cash.date_start}).compute(remain_amount, self.company.second_currency_id)
            self.second_rate = self.company.second_currency_id.round(convert > remain_amount and convert / remain_amount or remain_amount / convert)
            self.second_currency_value = convert
            
#         if cash.currency_id != cash.company_id.currency_id:
#             ex_rate, rate = self.pool.get('res.currency')._get_conversion_rate(self.cr, self.uid, cash.currency_id, cash.company_id.currency_id, context={'date':cash.date_start})
#             vay_rate = rate
#             self.currency_value = cash.company_id.currency_id.round(remain_amount * ex_rate)
#         if cash.company_id.second_currency_id and cash.currency_id != cash.company_id.second_currency_id:
#             ex_rate, rate = self.pool.get('res.currency')._get_conversion_rate(self.cr, self.uid, cash.currency_id, cash.company_id.second_currency_id, context={'date':cash.date_start})
#             self.second_rate = rate
#             self.second_currency_value = cash.company_id.second_currency_id.round(remain_amount * ex_rate)
        if cash.company_id.second_currency_id and cash.currency_id == cash.company_id.second_currency_id:
