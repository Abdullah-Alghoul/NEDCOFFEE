...
class wizrad_si(models.TransientModel):
    ...
    wizard_ids = fields.One2many('wizard.si.line', 'wizard_id', 'Wizard Lines', required=False)
    ...
    def button_convert(self):
        ...
        vars = {'name': self.name, 'final_destination': scontract.partner_shipping_id.id or False, 'partner_id': scontract.partner_id.id or False,
             'port_of_loading': scontract.port_of_loading.id or False, 'deadline': scontract.deadline or False, 'company_id': company or False,
             'port_of_discharge': scontract.port_of_discharge.id or False, 'shipment_from': warehouse_id.partner_id.id or False,
             'contract_id': scontract.id or False, 'default_type': 'export', 'warehouse_id': warehouse_id.id or False, 'type': 'export'}
...
class wizard_si_line(models.TransientModel):
    ...
    wizard_id = fields.Many2one('wizard.si', 'Wizard SI', required=False)
