...
class PurchaseContractLine(models.Model):
    ...
    invoice_lines = fields.One2many('account.invoice.line', 'contract_line_id', string="Invoice Lines", readonly=True, copy=False)
    
class AccountInvoice(models.Model):
    _inherit = 'account.invoice'

    contract_id = fields.Many2one('purchase.contract', string='Add Purchase Order',
    ...
    @api.onchange('state', 'partner_id', 'contract_line_ids')
    def _onchange_allowed_purchase_ids(self):
        ...
        contract_line_ids = self.invoice_line_ids.mapped('contract_line_id')
        contract_ids = self.invoice_line_ids.mapped('contract_id').filtered(lambda r: r.order_line <= contract_line_ids)
    ...
    def contract_order_change(self):
        ...
        for line in self.contract_id.order_line:
            ...
            invoice_line_tax_ids = self.contract_id.fiscal_position_id.map_tax(taxes)
            data = {
                'contract_line_id': line.id,
                'name': line.name,
                'origin': self.contract_id_id.origin,
            ...
            account = new_lines.get_invoice_line_account('in_invoice', line.product_id, self.contract_id.fiscal_position_id, self.env.user.company_id)
            if account:
        ...
        self.contract_id_id = False
        return {}

    @api.onchange('currency_id')
    def _onchange_currency_id(self):
        if self.currency_id:
            for line in self.invoice_line_ids.filtered(lambda r: r.contract_line_id):
                line.price_unit = line.contract_id.currency_id.compute(line.contract_line_id.price_unit, self.currency_id, round=False)

    @api.onchange('invoice_line_ids')
    def _onchange_origin(self):
        contract_ids = self.invoice_line_ids.mapped('contract_id')
        if contract_ids:
            self.origin = ', '.join(contract_ids.mapped('name'))
...
class AccountInvoiceLine(models.Model):
    ...
    contract_line_id = fields.Many2one('purchase.contract.line', 'Purchase Order Line', ondelete='set null', select=True, readonly=True)
    contract_id = fields.Many2one('purchase.contract', related='contract_line_id.contract_id', string='Purchase Contract', store=False, readonly=True,
        help='Associated Purchase contract. Filled in automatically when a PO is chosen on the vendor bill.')
