...
class Parser(rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.sum_money_manager = 0.0
        self.sum_tu_manager = 0.0
        self.sum_money_receipt_manager = 0.0
        ...
        self.localcontext.update({
            ...
            'money_text_manager': self.money_text_manager,
            'money_text_officier': self.money_text_officier,
            'money_text_worker': self.money_text_worker,
            '_count_manager': self._count_manager,
            '_count_officier': self._count_manager,
            '_count_worker': self._count_manager,
            'number_text_manager': self.number_text_manager,
            'number_text_officier': self.number_text_officier,
            'number_text_worker': self.number_text_worker,
        })
    ...
    def money_text_manager(self):
        return amount_to_text_vn.amount_to_text(self.get_sum()['money_manager'])
    def money_text_officier(self):
        return amount_to_text_vn.amount_to_text(self.get_sum()['money_officier'])
    def money_text_worker(self):
        return amount_to_text_vn.amount_to_text(self.get_sum()['money_worker'])
    
    def number_text_manager(self):
        str = amount_to_text_vn.vietnam_number(self.get_sum()['count_manager'])
        return str[0].upper() + str[1:]
    def number_text_officier(self):
        str = amount_to_text_vn.vietnam_number(self.get_sum()['count_officier'])
        return str[0].upper() + str[1:]
    def number_text_worker(self):
        str = amount_to_text_vn.vietnam_number(self.get_sum()['count_worker'])
        return str[0].upper() + str[1:]
    
    def _count_manager(self):
        return self.count_manager
    def _count_officier(self):
        return self.count_officier
    ...
    def get_sum(self):
        res = []
        return {'money_manager': self.sum_money_manager,
                'tu_manager': self.sum_tu_manager,
                'money_receipt_manager': self.sum_money_receipt_manager,
                'money_officier': self.sum_money_officier,
                ...
                'count_officier': self.count_officier
                }
...
#         payslip_structure_ids = payslip_structure_pool.search(self.cr, self.uid, [('code','=','VNCN')])
        ...
        if self.pool.get('ir.model.access').check_groups(self.cr, self.uid, "ned_general_hr_security.group_hr_manager_68z"):
            payslip_ids_sql = """select distinct hr_payslip.id FROM hr_payslip, hr_employee, insurance_code, hr_contract,resource_calendar
                                                        ...
                                                        AND hr_payslip.employee_id = hr_contract.employee_id AND hr_contract.working_hours = resource_calendar.id AND resource_calendar.unskilled_worker is not True""" % (
...
#                                 AND struct_id = %s""" % (self.date_from, self.date_to, payslip_structure_ids[0])
            ...
            for payslip_id in self.cr.fetchall():
                ...
                name_cap = self.pool.get('wizard.export.payslips.bank').strip_accents(name_emp).upper()
                ...
                if not bank_bic:
                    bank_bic =''
                res.append({'employee_name': payslip_obj.employee_id.name_related or False,
                            'name_cap': name_cap.replace(u"Đ", "D"),
                            'bank_number': payslip_obj.employee_id.bank_account_id.acc_number or False,
                            'bank': bank_name or False,
                            'bic':bank_bic or False,
                            'money': self.get_payslip_line(payslip_obj)['tl'],
                            'tu': self.get_payslip_line(payslip_obj)['tu'] * (-1) or False,
                            'money_receipt': self.get_payslip_line(payslip_obj)['tl'] - self.get_payslip_line(payslip_obj)['tu'],
                            })
                self.sum_money_manager += self.get_payslip_line(payslip_obj)['tl']
                self.sum_tu_manager += self.get_payslip_line(payslip_obj)['tu'] * (-1)
                self.sum_money_receipt_manager += self.get_payslip_line(payslip_obj)['tl'] - self.get_payslip_line(payslip_obj)['tu']
        return res 
...
#         payslip_structure_ids = payslip_structure_pool.search(self.cr, self.uid, [('code','=','VNCN')])
        ...
        for payslip_id in self.cr.fetchall():
            ...
            if payslip_obj.employee_id.bank_account_id.bank_id.name == 'Techcombank':
                ...
                self.sum_money_worker += self.get_payslip_line(payslip_obj)['tl']
                self.sum_tu_worker += self.get_payslip_line(payslip_obj)['tu'] * (-1)
                self.sum_money_receipt_worker += self.get_payslip_line(payslip_obj)['tl'] - self.get_payslip_line(payslip_obj)['tu']
        return res 
