...
#     
    ...
    def get_holidays(self, obj):
        ...
        if self.get_work100(obj.worked_days_line_ids)['day'] == 0.0:
            return {'quantity_paid': quantity_paid,
                    ...
                    'total_unpaid': 0.0,}
        return {'quantity_paid': quantity_paid,
                'total_paid': quantity_paid * obj.contract_id.wage/obj.default_work_days,
                'quantity_unpaid': quantity_unpaid,
                'total_unpaid': quantity_unpaid * obj.contract_id.wage/obj.default_work_days,}
    
    def get_payslip_line(self, obj):
        tu = ns = total_ns = pc = lgtt = 0
    ...
    def get_total(self, obj):
        wage = self.get_payslip_line(obj)['lgtt']
        ...
        return wage + total_150 + total_200 + total_300 + total_unpaid
        ...
        for payslip_id in self.cr.fetchall():
            ...
            if self.get_work100(payslip_obj.worked_days_line_ids)['day'] == 0.0:
                res.append({'employee_name': payslip_obj.employee_id.name_related,
                            'default_work_days': payslip_obj.default_work_days,
                        ...
                        'ns': self.get_payslip_line(payslip_obj)['ns'],
                        'lgtt': self.get_payslip_line(payslip_obj)['lgtt'],
                        'total_ns': self.get_payslip_line(payslip_obj)['total_ns'],
                        'get_total_disbured': self.get_total(payslip_obj) + self.get_holidays(payslip_obj)['quantity_paid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days + self.get_holidays(payslip_obj)['quantity_unpaid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days + self.get_payslip_line(payslip_obj)['total_ns'] + self.get_payslip_line(payslip_obj)['pc'],
                        'get_total': self.get_total(payslip_obj) + self.get_holidays(payslip_obj)['quantity_paid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days + self.get_holidays(payslip_obj)['quantity_unpaid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days - self.get_payslip_line(payslip_obj)['tu'] + self.get_payslip_line(payslip_obj)['total_ns'] + self.get_payslip_line(payslip_obj)['pc'],
                        })
            else:
                res.append({'employee_name': payslip_obj.employee_id.name_related,
                            'default_work_days': payslip_obj.default_work_days,
                            ...
                            'hour_work100': self.get_work100(payslip_obj.worked_days_line_ids)['hour'],
                            'contract_wage': payslip_obj.contract_id.wage,
                            'quy_doi_ngay': payslip_obj.contract_id.wage/payslip_obj.default_work_days or 0.0,
                            'quy_doi_gio': payslip_obj.contract_id.wage/payslip_obj.default_work_days/8 or 0.0,
                            'date_from': self.get_vietname_date(self.date_from),
                            'date_to': self.get_vietname_date(self.date_to),
                            'job': payslip_obj.employee_id.job_id.name,
                            'joining_date': self.get_vietname_date(payslip_obj.employee_id.joining_date),
                            ...
                            'quantity_paid': self.get_holidays(payslip_obj)['quantity_paid'],
                            'quantity_unpaid': self.get_holidays(payslip_obj)['quantity_unpaid'],
                            'total_paid': self.get_holidays(payslip_obj)['quantity_paid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days or 0.0,
                            'total_unpaid': self.get_holidays(payslip_obj)['quantity_unpaid'] * payslip_obj.contract_id.wage/payslip_obj.default_work_days or 0.0,
                            'unit_normal_ot': payslip_obj.contract_id.wage/payslip_obj.default_work_days/8*1.5 or 0.0,
                            'unit_rest_ot': payslip_obj.contract_id.wage/payslip_obj.default_work_days/8*2 or 0.0,
                            'unit_holiday_ot': payslip_obj.contract_id.wage/payslip_obj.default_work_days/8*4 or 0.0,
                            'pc': self.get_payslip_line(payslip_obj)['pc'],
                            'tu': self.get_payslip_line(payslip_obj)['tu'] * (-1),
                            'ns': self.get_payslip_line(payslip_obj)['ns'],
                            'lgtt': self.get_payslip_line(payslip_obj)['lgtt'],
